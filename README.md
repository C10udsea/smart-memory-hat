...
面向“空间性遗忘”问题的可穿戴视觉记忆辅助系统。帽体采集周围画面，云端做语义理解与向量检索，用户可以用自然语言查询，也可以为关键物品设置主动提醒。
...
系统解决的是用户“看见过、放过、经过过，但后来想不起来”的问题。帽体自动采集周围画面，云端完成特征提取和向量入库；用户可以像对话一样提问，系统返回答案并附带历史画面作为依据。同时，用户可以为药盒、钥匙、证件等关键物品录入提醒，当摄像头再次拍到该物品时触发提示。

目标用户包括阿尔茨海默患者、记忆衰退老人、家庭照护场景，以及任务切换频繁、容易忘记物品位置或行为是否完成的普通用户。
...
流程：帽体采集场景图像 → 云端语义理解与向量入库 → 用户自然语言提问 → 返回回答与图片证据。

### 2. 主动记忆提醒

用于“关键物品再次出现时主动提示”。

流程：用户录入目标物品与提醒内容 → 目标向量入库 → 后续图像相似度匹配 → 超过阈值后语音提醒。
...
### `POST /add_reminder`（待实现）

录入目标物品与提醒内容，用于主动记忆提醒。前端已预留调用，后端接口尚未实现。
...
这些目录由程序在运行时生成，仓库只保留目录结构（`.gitkeep` 占位），实际数据已通过 `.gitignore` 排除提交。
...
当前为原型样机验证阶段，已具备后端检索链路、网页端交互和单相机硬件采集代码。后续待完善：

- [ ] 后端补齐 `/add_reminder` 主动提醒接口与目标匹配逻辑
- [ ] `/query_memory` 返回图片证据（`evidence_image`）
- [ ] 硬件上传改为指向项目后端 `/upload_frame`，并支持四路摄像头与方向元数据
- [ ] 增加 API 鉴权、限流与上传大小校验
- [ ] 替换为支持中文的 CLIP 模型
- [ ] 补充 `.env.example` 与依赖版本锁定
